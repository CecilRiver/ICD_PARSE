...
IEC 61850 ICD/CID 文件信息提取工具，用于从智能电子设备(IED)配置文件中提取结构化信息，并基于IEC 61850-2标准对解析结果进行语义解析。
...
├── icd_parse/            # 核心解析模块
│   ├── __init__.py       # 包入口
│   ├── parser.py         # ICDParser 类（单文件解析）
│   ├── extractor.py      # ICDExtractor 类（批量提取）
...
```bash
# 摘要模式（默认） - 生成统计报告和语义对照表
python run.py input/61850ICD

# 完整模式 - 每个文件单独保存JSON
python run.py input/61850ICD -m full

# 语义解析模式 - 完整提取 + 语义标注
python run.py input/61850ICD -m semantic

# 仅统计
python run.py input/61850ICD -m stats

# 显示详细信息
python run.py input/61850ICD -v

# 显示缩写参考表
python run.py --show-reference

# 禁用语义解析
python run.py input/61850ICD --no-semantic

# 指定输出目录
python run.py input/61850ICD -o output
```
...
# 解析单个文件（启用语义解析）
parser = ICDParser('path/to/file.icd', enable_semantic=True)
result = parser.extract_all()

# 查看逻辑节点语义解析
for ld in result['lDevices']['lDevices']:
    if 'ln0' in ld:
        semantic = ld['ln0'].get('lnClass_semantic')
        print(f"LLN0 -> {semantic['zh']}")  # 输出: 逻辑节点零

# 批量提取
extractor = ICDExtractor('input/61850ICD', 'output')
